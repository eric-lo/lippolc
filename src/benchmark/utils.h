...
void set_affinity(uint32_t idx, bool hyper_thread = false) {
    cpu_set_t my_set;
    CPU_ZERO(&my_set);

    if (hyper_thread) {
        int socket_id = idx / 48;
        int cpu_id_in_socket = idx % 48;
        if (cpu_id_in_socket < 24) {
            CPU_SET(cpu_id_in_socket + socket_id * 24, &my_set);
        } else {
            CPU_SET(cpu_id_in_socket + socket_id * 24 + 72, &my_set);
        }
    } else {
        CPU_SET(idx, &my_set);
    }
