...
public:
    ...
    restart:
        ...
        //  if root readlock is on, somebody is changing root, I restart and come back later
        ...
        while (true)
        {
            ...
            if (parent)
            {                                                            // unless this is the 1st time (root), otherwise the child from the prev iteration would have already been locked before the end of the loop
            ...
            versionParent = versionNode; // initialize versionparent to be the fetched version number of the (new) parent
            ...
            {                                       // 1 means child
                ...
                versionNode = node->readLockOrRestart(needRestart); // set versionNode be child's version; read "lock" the child
                if (needRestart)
            ...
            { // the entry is a data
                if (skip_existence_check)
                {
                    if (parent)
                    {                                                            // unlock the parent
                        parent->readUnlockOrRestart(versionParent, needRestart); // echo ^; unlock the read lock of parent; in fact can unlock earlier
                        if (needRestart)
                            goto restart;
                    }
                    node->readUnlockOrRestart(versionNode, needRestart); // as this is the leaf node, unlock it
                    if (needRestart)
                        goto restart;
